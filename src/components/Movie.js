    ...
    <div className="mainSection">
      <div className="movieArticle">
        <div className="titleFlex">
          <h3 className="todaysTit">신규개봉 영화</h3>
        </div>
        <Swiper
          className="movieList swiper-initialized swiper-horizontal swiper-pointer-events"
          {...a11ys}
          slidesPerView={1}
          grid={{
            rows: 7,
        ...
        >
          {program.map((item) => (
            <SwiperSlide key={item.id}>
              <div className="toNavi" onClick={() => evtClick(item.id)}>
                <img src={imageUrl + item.poster_path} alt={item.title} />
                <h3 className="itemTit">
